...
PDF_LAYOUT_NOISE_RE = re.compile(r"\.indd\b", re.IGNORECASE)
PDF_TIMESTAMP_RE = re.compile(r"^\d{4}/\d{2}/\d{2}\s+\d{1,2}:\d{2}:\d{2}$")
PDF_MEANINGLESS_RE = re.compile(r"^[\d\s\-\._:/()]+$")
...
def cleanup_pdf_record_text(text: str) -> str:
    if not text:
        return ""
    ...
    while lines and PDF_PAGE_NUMBER_RE.fullmatch(lines[0]):
        lines.pop(0)

    while lines and PDF_PAGE_NUMBER_RE.fullmatch(lines[-1]):
        lines.pop()

    cleaned: list[str] = []
    for line in lines:
        if PDF_LAYOUT_NOISE_RE.search(line):
            continue
        if PDF_TIMESTAMP_RE.fullmatch(line):
            continue
        cleaned.append(line)
...
def is_meaningful_pdf_text(text: str) -> bool:
    ...
    if len(s) < 8 and PDF_MEANINGLESS_RE.fullmatch(s):
        return False

    if len(s) < 20 and PDF_PAGE_NUMBER_RE.fullmatch(s):
        return False
        ...
        for record in records:
            ...
            if isinstance(record, dict):
                if content_kind == "csv":
                    ...
                    text = cleanup_pdf_record_text(str(record.get("text") or ""))
                else:
            ...
            if content_kind == "pdf" and not is_meaningful_pdf_text(text):
                continue
